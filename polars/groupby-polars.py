...
# Stream the CSV straight to IPC without ever materializing the full frame in RAM.
# sink_ipc runs the streaming engine, so peak memory stays bounded regardless of file size.
# compression="uncompressed" keeps the IPC file zero-copy mmap-able for the queries below.
with pl.StringCache():
    (pl.scan_csv(src_grp, schema_overrides={"id4":pl.Int32, "id5":pl.Int32, "id6":pl.Int32, "v1":pl.Int32, "v2":pl.Int32, "v3":pl.Float64})
       .with_columns(pl.col(["id1", "id2", "id3"]).cast(pl.Categorical))
       .sink_ipc(f"{spill_dir}/tmp.ipc", compression="uncompressed"))
...
# Keep x lazy and backed by the memory-mapped IPC file; the queries below collect with the
# streaming engine so group-by state can spill to disk instead of exhausting RAM.
x = pl.scan_ipc(f"{spill_dir}/tmp.ipc", memory_map=True)

# materialize row count without pulling the whole frame into memory
...
ans = x.group_by("id1").agg(pl.sum("v1").alias("v1_sum")).collect(engine="streaming")
...
gc.collect()
t_start = timeit.default_timer()
ans = x.group_by(["id1","id2"]).agg(pl.sum("v1").alias("v1_sum")).collect(engine="streaming")
print(ans.shape, flush=True)
t = timeit.default_timer() - t_start
m = memory_usage()
t_start = timeit.default_timer()
...
gc.collect()
t_start = timeit.default_timer()
ans = x.group_by("id3").agg([pl.sum("v1").alias("v1_sum"), pl.mean("v3").alias("v3_mean")]).collect(engine="streaming")
print(ans.shape, flush=True)
t = timeit.default_timer() - t_start
m = memory_usage()
t_start = timeit.default_timer()
...
gc.collect()
t_start = timeit.default_timer()
ans = x.group_by("id4").agg([pl.mean("v1").alias("v1_mean"), pl.mean("v2").alias("v2_mean"), pl.mean("v3").alias("v3_mean")]).collect(engine="streaming")
print(ans.shape, flush=True)
t = timeit.default_timer() - t_start
m = memory_usage()
t_start = timeit.default_timer()
...
gc.collect()
t_start = timeit.default_timer()
ans = x.group_by("id6").agg([pl.sum("v1").alias("v1_sum"), pl.sum("v2").alias("v2_sum"), pl.sum("v3").alias("v3_sum")]).collect(engine="streaming")
print(ans.shape, flush=True)
t = timeit.default_timer() - t_start
m = memory_usage()
t_start = timeit.default_timer()
...
gc.collect()
t_start = timeit.default_timer()
ans = x.group_by(["id4","id5"]).agg([pl.median("v3").alias("v3_median"), pl.std("v3").alias("v3_std")]).collect(engine="streaming")
print(ans.shape, flush=True)
t = timeit.default_timer() - t_start
m = memory_usage()
t_start = timeit.default_timer()
...
gc.collect()
t_start = timeit.default_timer()
ans = x.group_by("id3").agg([(pl.max("v1") - pl.min("v2")).alias("range_v1_v2")]).collect(engine="streaming")
print(ans.shape, flush=True)
t = timeit.default_timer() - t_start
m = memory_usage()
t_start = timeit.default_timer()
...
gc.collect()
t_start = timeit.default_timer()
ans = x.drop_nulls("v3").group_by("id6").agg(col("v3").top_k(2).alias("largest2_v3")).explode("largest2_v3").collect(engine="streaming")
print(ans.shape, flush=True)
t = timeit.default_timer() - t_start
m = memory_usage()
t_start = timeit.default_timer()
...
gc.collect()
t_start = timeit.default_timer()
ans = x.group_by(["id2","id4"]).agg((pl.corr("v1","v2", method="pearson")**2).alias("r2")).collect(engine="streaming")
print(ans.shape, flush=True)
t = timeit.default_timer() - t_start
m = memory_usage()
t_start = timeit.default_timer()
...
gc.collect()
t_start = timeit.default_timer()
ans = x.group_by(["id1","id2","id3","id4","id5","id6"]).agg([pl.sum("v3").alias("v3"), pl.len().alias("count")]).collect(engine="streaming")
print(ans.shape, flush=True)
t = timeit.default_timer() - t_start
m = memory_usage()
t_start = timeit.default_timer()
